        ...
        if (empty($this->partsInfo)) {
            ...
            foreach ($result as $row) { 
                ...
                if (!isset($this->partsInfo[$part])) {
                    ...
                    if ($this->_proj == 'c490' || $this->_proj == 'cd391' || ($this->_proj == 'b515' && !$this->partsInfo[$part]["isAnte"])) {
                        $this->partsInfo[$part]["immediateProdStock"] = 0;
                    } else {
...
//                     }
                ...
                if (!$this->partsInfo[$part]["isMrp"]) {
                    ...
                    if ($row['drps_date'] != null && !empty($row['drps_qty']) && !isset($this->partsInfo[$part]['prods'][$row['drps_date']])) {
                        if ($row['is_ante'] === true) {
                            if ($row["drps_extra"] == 'ante') {
                                $this->partsInfo[$part]['prods'][$row['drps_date']] =  floatval($row['drps_qty']);
                            }
                        } else if ($row['is_ante'] === false) {
                            if ($row["drps_extra"] == 'ford') {
                                $this->partsInfo[$part]['prods'][$row['drps_date']] =  floatval($row['drps_qty']);
                            }
                        } else {
                            $this->partsInfo[$part]['prods'][$row['drps_date']] =  floatval($row['drps_qty']);
...
//                         else {
//                             $fwqty = $this->partsInfo[$part]["dmds"][$aedate];
//                             array_unshift($decompDates, $aedate);
//                             $deCounts = count($decompDates);
//                             foreach ($decompDates as $date) {
//                                 if (floor($fwqty / $deCounts)) {
//                                     $this->partsInfo[$part]["dmds"][$date] = floor($fwqty / $deCounts);
...
//             }
    ...
    public function DoAssemblyMrp ()
    {
        ...
        foreach ($this->activeDayDates as $curDate) {
            ...
            if ($this->_proj == 'c490') {
                if ($curDate > $startActiveDate) {
                    ...
                    foreach ($unmrpedParts as $part) {
                        $prodDate = $prevDate;
                        $prevOverallStock = $this->partsInfo[$part]["stocks"][$prodDate];
                        $curDmd = $this->partsInfo[$part]["dmds"][$curDate];
                        $preProd = $this->partsInfo[$part]["prods"][$prodDate];
                        ...
                        if ($this->isWorkday($curDate) && $this->partsInfo[$part]["hasDmds"]) {
                            ...
                            if ($netDmd > 0) {
                                ...
                                $this->arrangeMinExtraAssyProduction($part, $prodDate, $netDmd, false);
                            }
                        }
            ...
            } else if ($this->_proj == 'cd391' || $this->_proj == 'b515') {
                if ($curDate > $start2ActiveDate) {
                    $unmrpedParts = $this->getUnmrpedPartsByPredictedProdPriority($curDate, $this->saftyDayLength);

                    foreach ($unmrpedParts as $part) {
                        // B515的安特来源物料不考虑安全库存覆盖，只考虑将后一天需求提前即可
                        if ($this->partsInfo[$part]["isAnte"] && $this->isWorkday($curDate) && $this->partsInfo[$part]["hasDmds"]) {
                            $nextDate = $this->getNextWorkDate($curDate);
                            if ($this->isDayDate($nextDate)) {
                                $netProd = $this->partsInfo[$part]["dmds"][$nextDate] - $this->partsInfo[$part]["prods"][$curDate];
                                if ($netProd > 0) {
                                    $this->addPartProdOfDate($part, $curDate, $netProd);
                                }
                            }
                            $this->recalculateTotalDayProdOfDate($curDate);
                        ...
                        $curDmd = $this->partsInfo[$part]["dmds"][$curDate];
                        $pre2Prod = $this->partsInfo[$part]["prods"][$prodDate];
                        ...
                        if ($this->isWorkday($curDate) && $this->partsInfo[$part]["hasDmds"]) {
                            // 使用外库的净需求量计算方式存疑！！！
                            $netDmd = $consectiveDmdsAccu + $curDmd - $pre2Prod - $prevOverallStock;
...
//                     // 当前日期是可生产日(而不能只是有需求的可生产日，否则极端情况下会导致死循环)，且该零件有总需求(此时才有必要排计划)，才考虑“补充”进行最小生产性生产，来保证库存满足N天连续需求日覆盖
//                     if (
//                             $this->isWorkday($curDate) 
//                             //&& $this->isDmdDay($curDate) 
//                             && $this->partsInfo[$part]["hasDmds"]) {
...
//                             }
//                             // 使用外库的净需求量计算方式存疑！！！
//                             $netDmd = $consectiveDmdsAccu + $curDmd - $preProd - $prevOuterStock - $prevProd;
//                         } else {
//                             $netDmd = $consectiveDmdsAccu + $curDmd - $preProd - $prevOverallStock;
//                         }
...
//             }
            ...
            // 但如果在前一个需求日和本日之间，已经有了有排产的日期，则取消该规则，避免重复参考性排产。
            $lastExistingDmdDate = $this->getLastExistingDmdDate($curDate);
//             if ($this->prodRefLastExistingTotalDmd && $lastExistingDmdDate) {
                
//                 // 总产量相对于前需求日可略大或可略小的标志，逐日轮换。
//                 $initAlt = !$initAlt;
...
//                                    (
//                                     ($initAlt && $this->totalDayProds[$curDate] > $this->totalDayDmds[$lastExistingDmdDate] - $minOrdMin)
//                                     || 
//                                     (!$initAlt && $this->totalDayProds[$curDate] > $this->totalDayDmds[$lastExistingDmdDate] + $minOrdMin)
//                                     ) 
//                                     )
...
//                                 $netProd = $lastDmdDateAmountOfPallets - $this->partsInfo[$part]["prods"][$curDate];
//                                 $this->addPartProdOfDate($part, $curDate, $netProd);
//                             }
...
//                         $dmdedUnmrpedPartsForPlan = [];
//                         foreach ($dmdedUnmrpedParts as $part) {
//                             if ($this->partsInfo[$part]["dmds"][$lastExistingDmdDate]) {
//                                 $dmdedUnmrpedPartsForPlan[] = $part;
//                             }
//                         }

//                         foreach ($dmdedUnmrpedPartsForPlan as $part) {
//                             if ($this->availDayCapacities[$curDate] < $maxOrdMin) {
//                                 break;
//                             }
...
//                             $part = $this->getLeastDayProdPartBetween($curDate, $dmdedUnmrpedPartsForPlan);
//                             $this->addPartProdOfDate($part, $curDate, $this->partsInfo[$part]["ordMin"]);
...
//             }
            ...
            if ($this->leadDayLen >= 1) {  
                foreach ($dmdedUnmrpedParts as $part) {
                    if ($this->isWorkday($curDate)) {
                        $prevStock = $this->partsInfo[$part]["stocks"][$prevDate];
                        $curDmd = $this->partsInfo[$part]["dmds"][$curDate];
                        $preProd = $this->partsInfo[$part]["prods"][$curDate];
                        $relatedDmd = $this->getDmdAfterLeadedDay($part, $curDate);
                        $curNetDmd = $curDmd + $relatedDmd - $preProd - $prevStock ;
            ...
            if ($this->useComplement) {
                $this->complementDayCapacity($prodDate);
