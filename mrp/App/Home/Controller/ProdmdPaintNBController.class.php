...
class ProdmdPaintNBController extends ProdmdWeldCQController 
{
    ...
    protected function getProdAdapter()
    {
        if (empty($this->adapter)) {
            $this->adapter = new \Home\Model\PaintProdNBModel(1000, $_REQUEST["line"]);
        }
    ...
    protected function getProjAdapter()
    {
        if (empty($this->projAdapter)) {
            $this->projAdapter = new \Home\Model\PaintProjModel(1000);
        }
    ...
    {
        $adapter = new \Home\Model\PaintProdNBModel(1000);
    
        $adapter->exportBalanceExcel("涂装");
    }
