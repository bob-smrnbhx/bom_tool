...
class BomParserController extends Controller
{
    ...
    protected function addNormalDataFromZCode ()
    {
        $addedPartsInfo = [];
        foreach ($this->partsInfo as $code => $partInfo) {
            if (self::isDashZMaterialCode($code) && !isset($this->excludedParts[$code])) {
                $normalCode = substr($code, 0, -2);
                $partInfo["code"] = $normalCode;
                $addedPartsInfo[$normalCode] = $partInfo;
            }
        }
        
        $this->partsInfo = $addedPartsInfo + $this->partsInfo;
        ...
        foreach ($this->bomsInfo as $code => $bomInfo) {
            if (self::isDashZMaterialCode($code) && !isset($this->excludedParts[$code])) {
                $normalCode = substr($code, 0, -2);
                $addedBomsInfo[$normalCode] = $bomInfo;
                
                $this->altBomsInfo[$normalCode] = [
                        [$code, 1, 20]
                ];
            }
        }
        
        $this->bomsInfo = $addedBomsInfo + $this->bomsInfo;
    ...
    protected function generateCimFormatExcel ()
    {
        ...
        $sheetIndex = 0;
        // 1.4.3 `
        $this->generateExcelSheet_1_4_3($objPHPExcel, $sheetIndex, '1.4.3');

        
        // 1.16 
        $objPHPExcel->createSheet();
        $sheetIndex++;
        $this->generateExcelSheet_1_6($objPHPExcel, $sheetIndex, "1.16");
        ...
        if ($this->_bomLocs) {
            if (in_array("NB", $this->_bomLocs) && $this->altBomsInfo) {
                $objPHPExcel->createSheet();
                $sheetIndex++;
                $this->generateExcelSheet_13_1_Alt($objPHPExcel, $sheetIndex, "13.1-NB");
            }
            if (in_array("CQ", $this->_bomLocs)) {
                $objPHPExcel->createSheet();
                $sheetIndex++;
                $this->generateExcelSheet_13_1($objPHPExcel, $sheetIndex, "13.1-CQ", true);
            }
        }
        ...
        if ($this->_bomLocs) {
            if (in_array("NB", $this->_bomLocs)) {
                $objPHPExcel->createSheet();
                $sheetIndex++;
                $this->generateExcelSheet_13_5($objPHPExcel, $sheetIndex, "13.5-NB", false);
            }
            if (in_array("CQ", $this->_bomLocs)) {
                $objPHPExcel->createSheet();
                $sheetIndex++;
                $this->generateExcelSheet_13_5($objPHPExcel, $sheetIndex, "13.5-CQ", true);
            }
        }
        ...
        if ($this->altBomsInfo) {
            $objPHPExcel->createSheet();
            $sheetIndex++;
            $this->generateExcelSheet_13_15_Alt($objPHPExcel, $sheetIndex, "13.15-NB", false);
        }
        ...
        if ($this->_bomLocs) {
            if (in_array("NB", $this->_bomLocs)) {
                $objPHPExcel->createSheet();
                $sheetIndex++;
                 $this->generateExcelSheet_14_13_1($objPHPExcel, $sheetIndex, "14.13.1-NB", false);
            }
            if (in_array("CQ", $this->_bomLocs)) {
                $objPHPExcel->createSheet();
                $sheetIndex++;
                 $this->generateExcelSheet_14_13_1($objPHPExcel, $sheetIndex, "14.13.1-CQ", true);
            }
        }
        ...
        if ($this->altBomsInfo) {
            $objPHPExcel->createSheet();
            $sheetIndex++;
            $this->generateExcelSheet_14_15_1_Alt($objPHPExcel, $sheetIndex, "14.15.1");
        }
        ...
        if ($this->_genOaTpl) {
            $objPHPExcel->createSheet();
            $sheetIndex++;
            $this->generateExcelSheet_Oa_Circulation($objPHPExcel, $sheetIndex, "OA流转单-自制", "M");
            $objPHPExcel->createSheet();
            $sheetIndex++;
            $this->generateExcelSheet_Oa_Circulation($objPHPExcel, $sheetIndex, "OA流转单-外购", "B");
        }
    ...
    protected function generateExcelSheet_1_4_3 (\PHPExcel $objPHPExcel, $sheetIndex, $sheetTitle)
    {
        $objPHPExcel->setActiveSheetIndex($sheetIndex);
        $objActSheet = $objPHPExcel->getActiveSheet($sheetIndex);
...
//         $objActSheet->setConditionalStyles("C9", [$pValue]);
    ...
    protected function generateExcelSheet_1_6 (\PHPExcel $objPHPExcel, $sheetIndex, $sheetTitle)
    {
        $objPHPExcel->setActiveSheetIndex($sheetIndex);
        $objActSheet = $objPHPExcel->getActiveSheet($sheetIndex);
        $objActSheet->setTitle($sheetTitle);
        ...
        $r = 6;
        foreach ($this->partsInfo as $partInfo) {
            if (!isset($partInfo["custCode"]) || empty($partInfo["custCode"]) || self::isDashZMaterialCode($partInfo["code"])) {
                continue;
            }
...
//             }
    ...
    protected function generateExcelSheet_13_1 (\PHPExcel $objPHPExcel, $sheetIndex, $sheetTitle, $bomSuffix = '')
    {
        $objPHPExcel->setActiveSheetIndex($sheetIndex);
        $objActSheet = $objPHPExcel->getActiveSheet($sheetIndex);
        $objActSheet->setTitle($sheetTitle);
    ...
    protected function generateExcelSheet_13_1_Alt (\PHPExcel $objPHPExcel, $sheetIndex, $sheetTitle)
    {
        $objPHPExcel->setActiveSheetIndex($sheetIndex);
        $objActSheet = $objPHPExcel->getActiveSheet($sheetIndex);
        $objActSheet->setTitle($sheetTitle);
    ...
    protected function generateExcelSheet_13_5 (\PHPExcel $objPHPExcel, $sheetIndex, $sheetTitle, $bomSuffix = '')
    {
        $objPHPExcel->setActiveSheetIndex($sheetIndex);
        $objActSheet = $objPHPExcel->getActiveSheet($sheetIndex);
        $objActSheet->setTitle($sheetTitle);
    ...
    protected function generateExcelSheet_13_15_Alt (\PHPExcel $objPHPExcel, $sheetIndex, $sheetTitle)
    {
        $objPHPExcel->setActiveSheetIndex($sheetIndex);
        $objActSheet = $objPHPExcel->getActiveSheet($sheetIndex);
        $objActSheet->setTitle($sheetTitle);
    ...
    protected function generateExcelSheet_14_13_1 (\PHPExcel $objPHPExcel, $sheetIndex, $sheetTitle, $bomSuffix = '')
    {
        $objPHPExcel->setActiveSheetIndex($sheetIndex);
        $objActSheet = $objPHPExcel->getActiveSheet($sheetIndex);
        $objActSheet->setTitle($sheetTitle);
    ...
    protected function generateExcelSheet_14_15_1_Alt (\PHPExcel $objPHPExcel, $sheetIndex, $sheetTitle)
    {
        $objPHPExcel->setActiveSheetIndex($sheetIndex);
        $objActSheet = $objPHPExcel->getActiveSheet($sheetIndex);
        $objActSheet->setTitle($sheetTitle);
    ...
    protected function generateExcelSheet_Oa_Circulation (\PHPExcel $objPHPExcel, $sheetIndex, $sheetTitle, $type)
    {
        $objPHPExcel->setActiveSheetIndex($sheetIndex);
        $objActSheet = $objPHPExcel->getActiveSheet($sheetIndex);
        $objActSheet->setTitle($sheetTitle);
    ...
    public static function getMainNameOfDesc ($desc)
    {
        ...
        //return strtr($desc, "\r\n", "  ");
//         if (stripos($desc, "\r") > 0) {
//             return strstr($desc, "\r", true);
//         } else if (stripos($desc, "\n") > 0) {
//             return strstr($desc, "\n", true);
//         } else {
//             return $desc;
//         }
    ...
    public function parseExcel ($filename, $sheetIndex = 0, $maxParsedRow = 0)
    {
        ...
        // 载入文件
        $this->_curExcel = $PHPReader->load($filename);
        
        // 如果超出工作表最大序号，返回空白数组
        if ($sheetIndex > $this->_curExcel->getSheetCount() - 1) {
            return [];
        }
        
        // 获取表中的第一个工作表，如果要获取第二个，把0改为1，依次类推

        $this->_curSheet = $this->_curExcel->getSheet($sheetIndex);
        // 获取总列数
