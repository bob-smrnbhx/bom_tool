 ...
 *
 */
class ProDayMrpCheckController extends ProDayMrpController
{
    ...
    public function confirmVendOrder()
    {
        ...
        try {
            ...
            // 如果上一个版本号是今天的，则加+1即可, 否则，上一个是前日的版本号或尚无版本号，新建今日初始版本号，并且不能超过当日可能的最大版本号限定。
    ...
    public function rejectVendOrder()
    {
        if (!IS_AJAX) {
            throw new \Exception("invalid post data from none-ajax request");
        }
        ...
        try {
            ...
            $nbrCondStr = "tran_nbr is null";
            $ptpCondStr = "ptp_pm_code='P' AND ptp_status='AC'";
            ...
            $msg = "已驳回供应商: $vend 的采购日程数据，申购员需要重新确认";
        } catch (\Exception $e) {
            $tranModel->rollback();
            $err = true;
            $msg = $e->getMessage();
    ...
    public function confirmAllVendOrders ()
    {
