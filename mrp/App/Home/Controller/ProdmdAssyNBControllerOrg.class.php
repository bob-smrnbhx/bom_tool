...
//                 // 累计每个零件的活动日的862需求总需求量作为参考
    ...
    public function DoAssemblyMrp ()
    {
        ...
        foreach ($this->activeDayDates as $curDate) {
            ...
            // 首先确保每个物料的每日结余都必须大于最小包装量
            if ($this->isWorkday($curDate)) {
                foreach ($undmdedUnmrpedParts as $part) {
                    $prevDate = self::getDateBefore($curDate);
                    $prevOverallStock = $this->partsInfo[$part]["stocks"][$prevDate];
                    $ordMin = $this->partsInfo[$part]["ordMin"];

                    if ($prevOverallStock < $ordMin) {
                        $this->partsInfo[$part]["prods"][$curDate] += $ordMin;
                        $this->availDayCapacities[$curDate] -= $ordMin;
...
//         }
    ...
    public function exportBalanceExcel ()
    {
        ...
        $objActSheet->setTitle("$this->_proj 装配平衡表");
        ...
        ->setCellValue($logoStartCell, "$this->_proj-生产平衡表");
        ...
        foreach ($this->partsInfo as $part => $partInfo) {
            ...
            // 用若干合并的6列来写非日期相关信息：
            $objActSheet
            ->mergeCells("A$column:A" . ($column + 5))
            ->setCellValueExplicit("A$column", $part)
            ->mergeCells("B$column:B" . ($column + 5))
            ->setCellValueExplicit("B$column", $partInfo["line"])

            ->setCellValueExplicit("C" . $column, $partInfo["desc1"])
            ->setCellValueExplicit("C" . ($column + 1), $partInfo["desc2"])
            ->setCellValueExplicit("C" . ($column + 2), "初始内库库存: " . $partInfo["innerStocks"][$orgDate])
            ->setCellValueExplicit("C" . ($column + 3), "初始外库库存: " . $partInfo["outerStocks"][$orgDate])
            ->setCellValueExplicit("C" . ($column + 4), "每托个数：" . $partInfo["ordMin"])
            ->setCellValueExplicit("C" . ($column + 5), "前日需求量：" . $partInfo["dmds"][$orgDate])
            
            ->setCellValueExplicit("D" . $column, "需求")
            ->setCellValueExplicit("D" . ($column + 1), "计划")
            ->setCellValueExplicit("D" . ($column + 2), "内库")
            ->setCellValueExplicit("D" . ($column + 3), "外库 ")
            ->setCellValueExplicit("D" . ($column + 4), "总结余")
            ->setCellValueExplicit("D" . ($column + 5), "累积量")
            ...
            foreach ($activeDates as $date) {
                $objActSheet
                ->setCellValueExplicit($prefix . $j . $column,       floatval($partInfo["dmds"][$date]))
                ->setCellValueExplicit($prefix . $j . ($column + 1), floatval($partInfo["prods"][$date]))
                ->setCellValueExplicit($prefix . $j . ($column + 2), floatval($partInfo["innerStocks"][$date]))
                ->setCellValueExplicit($prefix . $j . ($column + 3), floatval($partInfo["outerStocks"][$date]))
                ->setCellValueExplicit($prefix . $j . ($column + 4), floatval($partInfo["stocks"][$date]))
                ->setCellValueExplicit($prefix . $j . ($column + 5), floatval($partInfo["consectAccuDmds"][$date]))
                ;
        ...
        $today = date("Y-m-d");
        $filename = "$this->_proj 装配平衡表($today).xls";
