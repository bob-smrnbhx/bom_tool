...
//     }
     ...
     * @return boolean
     */
    protected function isCommonWorkday ($date)
    {
        if (!$this->isDayDate($date)) {
            return true;
        }
    ...
    protected function getIsWorkdayDateMap ()
    {
        ...
        foreach ($this->getActiveDates() as $date) {
            $map[$date] = $this->isCommonWorkday($date);
        }
    ...
    public function getPartsInfo()
    {
        if (empty($this->partsInfo)) {
            ...
            foreach ($result as $row) { 
                ...
                if (!isset($this->partsInfo[$part]['dmds'][$row["dmd_date"]][$row["sod_ship"]])) {
                    if ($this->partsInfo[$part]["isAnte"] && $row["sod_ship"] == "3264H1") {
                        $this->partsInfo[$part]['dmds'][$row["dmd_date"]][$row["sod_ship"]] = floatval($row['dmd_qty']);
                        $this->partsInfo[$part]['dmdTypes'][$row["dmd_date"]] = floatval($row["dmd_type"]);
                    } else if (!$this->partsInfo[$part]["isAnte"] && $row["sod_ship"] != "3264H1") {
                        $this->partsInfo[$part]['dmds'][$row["dmd_date"]][$row["sod_ship"]] = floatval($row['dmd_qty']);
                        $this->partsInfo[$part]['dmdTypes'][$row["dmd_date"]] = floatval($row["dmd_type"]);
                    }
            ...
            foreach ($this->partsInfo as $part => $partInfo) {
                // 叠加所有订单的862需求量和初始需求量
                foreach ($this->partsInfo[$part]['dmds'] as $date => $dmds) {
                    $this->partsInfo[$part]['dmds'][$date] = array_sum($dmds);
                }
                ...
                $this->partsInfo[$part]["hasDayDmds"] = !empty($this->partsInfo[$part]["dmdsSum"]);
...
//             var_dump($date, $lastDmdDate, $this->totalDayDmds[$lastDmdDate]);
            var_dump($date, $this->isWorkableDayWithDmd($date));
            echo "<hr />";
        }
    }
...
//                         }
                        ...
                        $this->partsInfo[$part]["prods"][$curDate] = $lastDmdDateAmountOfPallets;
                ...
                // 再设定一个均衡算法，部分可超出，部分不超出，从而让总生产量接近前需求存在日总需求量，但复杂度极高……
                if ($this->isCommonWorkday($curDate) && $this->isDmdDay($curDate)) {
                    $minOrdMin = null;
                    $maxOrdMin = null;
                    foreach ($unmrpedparts as $part) {
                    ...
                    while ($this->availDayCapacities[$curDate] > $maxOrdMin && $this->totalDayDmds[$lastExistingDmdDate] - $this->totalDayProds[$curDate] >= $maxOrdMin  ) {
                        $curPart = $this->getLeastDayProdPartBetween($curDate, $unmrpedparts);
            ...
            if ($this->useSaftyStockRule) {  
                ...
                foreach ($unmrpedparts as $part) {
                    ...
                    if ($this->partsInfo[$part]["isAnte"] && $this->isCommonWorkday($curDate) && $this->partsInfo[$part]["hasDayDmds"]) {
                        $nextDate = self::getDateAfter($curDate);
                        if ($this->isDayDate($nextDate)) {
                            $this->partsInfo[$part]["prods"][$curDate] = $this->partsInfo[$part]["dmds"][$nextDate];
                            $this->availDayCapacities[$curDate] -= $this->partsInfo[$part]["dmds"][$nextDate];
                        }
                    ...
                    if ($this->isCommonWorkday($curDate) && $this->isDmdDay($curDate) && $this->partsInfo[$part]["hasDayDmds"]) {
            ...
            if ($this->leadDayLen >= 1) {  
                $prevDate = self::getDateBefore($curDate);
                foreach ($unmrpedparts as $part) {
                    if ($this->isCommonWorkday($curDate)) {
                        $prevStock = $this->partsInfo[$part]["stocks"][$prevDate];
                        $curDmd = $this->partsInfo[$part]["dmds"][$curDate];
    ...
    protected function arrangeMinExtraAssyProduction($part, $prodDate, $curNetDmd, $isBackdate)
    {
        ...
        if ($leftCapacity >= $curNetDmdForPallets) {
            ...
            $preProdDate = $this->getClassClosestWorkdayDate($this->partsInfo[$part]["class"], $prodDate, false);
    ...
    protected function calculatePartStockOfDate ($part, $date)
    {
        ...
        if (($this->isCommonWorkday($date) && $this->isDmdDay($date)) || ($this->isMinProdRequiredDay($date))) {
    ...
    public function getClassClosestWorkdayDate($class, $date, $allowCurrent = false)
    {
        if ($this->isWorkdayByClassAndDate($class, $date) && $allowCurrent) {
            return $date;
        }
        ...
        } while (!$this->isWorkdayByClassAndDate($class, $date));
